...
// if there are any large changes. Each sample is muliplied by a convolution
...
// These are point to sample relative to the starting point.
...
// This function runs the sobel algorithm over the depth texturej
void DepthSobel_float(float2 UV, float Thickness, out float Out) {
    float2 sobel = 0;
    // We can unroll this loop to make it more efficient
    // The compi;ler is also smart enough to remove the i=4 iteration, which is always zero
